...
- `json/` — schema-conformant extractions (source of truth):
  - `aws_sustainability_report_2025.json` — 2025 data-centre **withdrawal (9.4 billion L)**; withdrawal-based WUE **full series 2021–2025 (0.25 → 0.19 → 0.18 → 0.15 → 0.12 L/kWh)**; **water-stress shares via WRI Aqueduct** (48% low, 22% high/extremely high, 2030 BAU scenario); water-positive progress (53% 2024 → 75% 2025); replenishment returned (9.4 billion L in 2025; >18 billion L contracted); avoided/preserved water (938 M L cooling reduced; 849 M L potable preserved). 14 observations.
...
- **Withdrawal-based WUE** (full series 0.25 in 2021 → 0.19 → 0.18 → 0.15 → 0.12 in
  2025), global fleet average — not regional, site, or workload-attributed, and not Amazon
